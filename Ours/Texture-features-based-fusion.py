...
# Multi-model medical image fusion using texture feature and weighted fusion strategy
"""
...
class IFCNN(nn.Module):
    ...
    def forward(self, *tensors):
        ...
        # arr1=tensors[0][0][0].cpu().detach().numpy()
        # arr2=tensors[1][0][0].cpu().detach().numpy()
        # f1=fast_glcm.fast_glcm_mean(arr1)
        # f2=fast_glcm.fast_glcm_mean(arr2)
        # f1=np.expand_dims(f1,axis=0)
        # f1=np.expand_dims(f1,axis=0)
        # f2=np.expand_dims(f2,axis=0)
        # f2=np.expand_dims(f2,axis=0)
        # f1=torch.from_numpy(f1).to(device).float()
        # f2=torch.from_numpy(f2).to(device).float()
        # outs[0]=torch.cat((outs[0],f1),1)
        # outs[1]=torch.cat((outs[1],f2),1)
        # Feature fusion
        ...
        for sum_maps in zip(*imgs_sum_maps):
                #print(self.images_to_tensors[0])
                features = torch.cat(sum_maps, dim=0)
                weights = self._softmax(features)
    ...
    def fast_glcm(self,img, vmin=0, vmax=255, nbit=8, kernel_size=5):
        ...
        # kernel = torch.ones((1,ks, ks), dtype=torch.uint8)
        # for i in range(nbit):
        #     for j in range(nbit):
        #         glcm[i,j] = F.conv2d(glcm[i,j],kernel,1)
...
for epoch in range(EPOCH):
    ...
    for idx in range(batch_idxs):
        ...
        b_y = train_pet_tensor[idx].type(torch.float32).to(device)
...
#             %(epoch, counter, ssim_loss_mri, ssim_loss_pet, ssim_total, l2_total, loss_total))
    ...
    if(epoch%2==0):
      #Save a checkpoint
      torch.save(cnn.state_dict(), 'GLCM-mean.pth')
...
model = myIFCNN()
model.load_state_dict(torch.load('GLCM-mean.pth'))
model.eval()
model = model.cuda()
