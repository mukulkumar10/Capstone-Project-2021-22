...
Original file is located at
    https://colab.research.google.com/drive/1UOEz-g4WR3aDt2ylT_vBio1lLnJg_YTb

# IFCNN-SSIM implementation for multi-model brain image fusion
"""
...
class IFCNN(nn.Module):
    ...
    def tensor_max(self, tensors):
        ...
        for _,i in enumerate(range(64)):
              tensor1=tensors[0][0][i].to(device)
              tensor1=torch.unsqueeze(tensor1,0)
              tensor1=torch.unsqueeze(tensor1,0)
              #print(tensor1.size())
              tensor2=tensors[1][0][i].to(device)
              tensor2=torch.unsqueeze(tensor2,0)
              tensor2=torch.unsqueeze(tensor2,0)
              #print(tensor2.size())
              tensor3=train_gt_tensor[i].to(device)
              #print(tensor3.size())
              v1 = ssim(tensor3, tensor1, val_range=1) #SSIM Score of Ground Truth and MRI
              v2 = ssim(tensor3, tensor2, val_range=1) #SSIM Score of Ground Truth and CT
...
#             %(epoch, counter, ssim_loss_mri, ssim_loss_pet, ssim_total, l2_total, loss_total))
    ...
    if(epoch == EPOCH -1):
      #Save a checkpoint
      torch.save(cnn.state_dict(), 'IFCNN_SSIM.pth')
...
"""# Python code snippet for model testing"""

from PIL import Image

model=FunFuseAn()
model.load_state_dict(torch.load('IFCNN_SSIM.pth'))
model.eval()
model=model.cuda()
...
matplotlib.image.imsave('IFCNN_SSIM50.gif', ffused)
